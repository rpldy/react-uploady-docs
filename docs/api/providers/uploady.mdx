...
| listeners            | Object        | undefined | map of [uploader events](uploader#events) name and event handler |
...
Providers like [Chunked-Uploady](providers/chunked-uploady), use the ability to swap the send method in order to provide the one from their own sender.
ie: [Chunked-Sender](senders/chunkedSender).

### formatServerResponseMethod

A function to create the batch item's uploadResponse from the raw xhr response.

- See [FormatServerResponseMethod](types#formatserverresponsemethod)
...
and using the [useFileInput hook](hooks/useFileInput) to pass a ref to the custom element.
